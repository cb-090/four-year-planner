...
const springboxDiv = document.getElementById("springbox");


document.querySelector("body").addEventListener('click', (e) => {
  if (e?.target?.className === "fallClass") {
    console.log(e.target.id)
    const selected = document.getElementById(e.target.id);
    selected.remove();
    fallboxDiv.appendChild(selected);
  }
})

document.querySelector("body").addEventListener('click', (e) => {
  if (e?.target?.className === "springClass") {
    console.log(e.target.id)
    const selected = document.getElementById(e.target.id);
    selected.remove();
    springboxDiv.appendChild(selected);
  }
})
